...
#define REV_ATLAS_LITE_2_1         0x11
	...
	if (is_soc_rev(CHIP_REV_2_0) <= 0) {
		...
		/* Set core voltage to 1.15V */
		val = pmic_reg(slave, 24, 0, 0);
		val = (val & (~0x1f)) | 0x16;  // ( 0x16 -> 1.15V (MAX Voltage) , 0x14 -> 1.10V )
		pmic_reg(slave, 24, val, 1);

		/* TO 3.0 */
		/* Setup VCC (SW2) to 1.225 */
		val = pmic_reg(slave, 25, 0, 0);
		val = (val & (~0x1f)) | 0x19;  // ( 0x19 -> 1.225V, Ox1a -> 1.250V, 0x1b -> 1.275V ( MAX Voltage ) ) 
		pmic_reg(slave, 25, val, 1);

		/* Setup 1V2_DIG1 (SW3) to 1.2 */
		val = pmic_reg(slave, 26, 0, 0);
		val = (val & (~0x1f)) | 0x18;	// (0x18 -> 1.2V, 0x1a -> 1.250V, 0x1b -> 1.275V ( MAX voltage ) )
		pmic_reg(slave, 26, val, 1);

		/* System Reset */
//		val = pmic_reg(slave, 15, 0, 0);
//		val = (val & (~0x1000)) | 0x1000;
...
void get_board_serial(struct tag_serialnr *serialnr)
{
	...
	for (i = 0; i < 8; ++i, ++iim1_emei_base)
		emeival[i] = (u8)readl(iim1_emei_base);

	serialnr->low = emeival[7] | ( (u32)emeival[6]<<8) | ((u32)emeival[5]<<16) | ((u32)emeival[4]<<24);
	serialnr->high = emeival[3] | ( (u32)emeival[2]<<8) | ((u32)emeival[1]<<16) | ((u32)emeival[0]<<24);
}
#endif


static void lvds_backlight_off(void)				
{
	unsigned int reg;
...
#define USBCMD 0x140
	...
	gd->bd->bi_arch_number = MACH_TYPE_MX51_NA04;	/* board id for linux */
